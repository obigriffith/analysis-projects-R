#!/usr/bin/env Rscript
#Written by Malachi Griffith, modified by Obi Griffith
...
#############################################################################################################################################
plotChrCNV = function(target_chr,chr_start,chr_end,direction,cnvs,genes,segments,ideo_data,hard_cap_upper,hard_cap_lower){
  #Create plotting space
  lay = layout(matrix(c(1,2,3,4), ncol=2, byrow=F), heights=c(0.6,2), widths=c(0.2,1), FALSE) #layout.show(lay)
  ...
  plot.new()
  par(mar=c(0,0,0,0))
  legend("topright", col=ideo_legend_cols, pch=15, legend=ideo_legend_text, title="Giemsa staining", cex=1)
  ...
  par(mar=c(0, 0, 1.5, 4))
  drawIdeogram(ideo_data, target_chr, chr_start, chr_end)
  par(mar=c(3, 0, 1.5, 4))
  #par(mar=c(3, 4, 1.5, 0))
...
#############################################################################################################################################
drawIdeogram = function(ideo_data, target_chr, chr_start, chr_end){
...
#createPlot                                                                     #
#############################################################################################################################################
#Create a function to produce image
createPlot = function(chr,chr_start,chr_end,cnvs,genes,segments,ideo_data,image_type){
  ...
  for (chr_name in chr_list){
    if (image_type == "none"){
      plotChrCNV(chr_name,chr_start,chr_end,"gains",cnvs,genes,segments,ideo_data,hard_cap_upper,hard_cap_lower)
      plotChrCNV(chr_name,chr_start,chr_end,"losses",cnvs,genes,segments,ideo_data,hard_cap_upper,hard_cap_lower)
    }else{
      #Gains
...
loadData = function(patient){
  #Import patient common names and last succeeded builds to cycle through
  build_info_file="/Users/ogriffit/Dropbox/WashU/Projects/HCC/clinseq_last_succeeded_builds.txt"
  builds=read.table(build_info_file, sep="\t", as.is=c(1,2), header=TRUE, row.names=1)

  #Download needed files over https:
  tempdir="/Users/ogriffit/Dropbox/WashU/Projects/HCC/converge/HCC_30/CNV/ManualReview/temp"
  cnv_file_url = paste("https://gscweb.gsc.wustl.edu",builds[patient,"LAST_SUCCEEDED_BUILD_DATA_DIRECTORY"],"/",patient,"/cnv/cnvs.hq", sep="")
  gene_file_url = paste("https://gscweb.gsc.wustl.edu",builds[patient,"LAST_SUCCEEDED_BUILD_DATA_DIRECTORY"],"/",patient,"/cnv/cnview/CNView_CancerGeneCensusPlus_Sanger/CNView_CancerGeneCensusPlus_Sanger_genes.tsv", sep="")
  segments_file_url = paste("https://gscweb.gsc.wustl.edu",builds[patient,"LAST_SUCCEEDED_BUILD_DATA_DIRECTORY"],"/",patient,"/cnv/cnview/CNView_CancerGeneCensusPlus_Sanger/cnaseq.cnvhmm.tsv", sep="")
  cnv_file = paste(tempdir,"/cnvs.hq",sep="")
  gene_file = paste(tempdir,"/CNView_CancerGeneCensusPlus_Sanger_genes.tsv",sep="")
  segments_file = paste(tempdir,"/cnaseq.cnvhmm.tsv",sep="")
  download.file(cnv_file_url, destfile=cnv_file, method="wget", extra=c("--no-check-certificate"))
  download.file(gene_file_url, destfile=gene_file, method="wget", extra=c("--no-check-certificate"))
  download.file(segments_file_url, destfile=segments_file, method="wget", extra=c("--no-check-certificate"))
  ...
  return(list(cnvs,genes,segments,ideo_data))
}
...
#roundUp                                                                     #
#############################################################################################################################################
#round up to nearest 10000, since that is the size of windows on which underlying data is based
roundUp=function(x,to=10000){
  rounded=to*(x%/%to + as.logical(x%%to))
  if(rounded<10000){rounded=10000} #Range must start with at least first 10000 bin
  return(rounded)
}
...
#getDataToStore                                                                     #
#############################################################################################################################################
getDataToStore = function(CNV_segment,cnvs,patient,chr){
  #Calculate mean DIFF CNV for this region
  CNV_seg_start=roundUp(CNV_segment$x[1])-10000
  CNV_seg_end=roundUp(CNV_segment$x[2])
  if(CNV_seg_start<min(cnvs[cnvs[,"CHR"]==chr,"POS"])){CNV_seg_start=min(cnvs[cnvs[,"CHR"]==chr,"POS"])}
  if(CNV_seg_end>max(cnvs[cnvs[,"CHR"]==chr,"POS"])){CNV_seg_end=max(cnvs[cnvs[,"CHR"]==chr,"POS"])}
  mean_DIFF=round(mean(cnvs[cnvs[,"CHR"]==chr & cnvs[,"POS"]>=CNV_seg_start & cnvs[,"POS"]<=CNV_seg_end,"DIFF"]), digits=3)
  data_to_store=t(as.matrix(c(patient,chr,CNV_seg_start,CNV_seg_end,mean_DIFF)))
...
ideogram_file = "/Users/ogriffit/Dropbox/WashU/Projects/HCC/Rscripts/ChrBandIdeogram.tsv"
outdir = "/Users/ogriffit/Dropbox/WashU/Projects/HCC/converge/HCC_30/CNV/ManualReview"
image_type = "none"

#First import data for one patient
patient="HCC2"
data=loadData(patient)
cnvs=data[[1]]
genes=data[[2]]
segments=data[[3]]
ideo_data=data[[4]]
...
#Produce plots for manual review of one chromosome at a time
#First, create plot for whole chromosome
createPlot(chr,0,0,cnvs,genes,segments,ideo_data,image_type)

#Then, zoom into any regions of interest
#use locator to click region for zooming
zoompoints=locator()
createPlot(chr,roundUp(zoompoints$x[1]),roundUp(zoompoints$x[2]),cnvs,genes,segments,ideo_data,image_type)

#Capture a region of interest by manual selection of points in plot window (ESC to end locate session)
CNV_segment=locator()
data_to_store=getDataToStore(CNV_segment,cnvs,patient,chr)
write.table(data_to_store, file="manually_reviewed_segments.1.txt", append=TRUE, quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)
