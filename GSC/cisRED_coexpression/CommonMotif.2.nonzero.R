...
outfile="CommonModuleGeneClusters_2_results_Affy_nonzero_vs_random10000pairs_20.txt"

#Postscript file
ps_file="LogFreq_dist_rand_vs_norm_nonzero.ps"
...
#Get only non-zero results
data_norm_nonzero = data_norm[data_norm[,2]>0,]
data_rand_nonzero = data_rand[data_rand[,2]>0,]
...
data_norm_for_clustersize = data_norm_nonzero[data_norm_nonzero[,1]==clustersize,]
data_rand_for_clustersize = data_rand_nonzero[data_rand_nonzero[,1]==clustersize,]
...
hist_df = hist(data_norm_nonzero[,2], nclass=1000, plot=FALSE)
hist_df_rand = hist(data_rand_nonzero[,2], nclass=1000, plot=FALSE)
