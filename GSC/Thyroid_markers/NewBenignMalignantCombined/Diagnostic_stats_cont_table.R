...
setwd("C:/Documents and Settings/obig/My Documents/Projects/Thyroid_markers/NewBenignMalignantCombined")
datafile="NewThyroidCANCERTMA_CC_KK_Clean_NOV3009.txt"

outfile="output_test.txt"

#Read in data (expecting a tab-delimited file with header line and rownames)
data=read.table(file=datafile, header = TRUE, na.strings = "NA", sep="\t", as.is=TRUE)
...
#Get marker data
marker_data=data[,c(12,14,15)] #ungrouped values
...
for (i in 1:length(marker_data)){
   ...
   Benign_scores=marker_data[target=="B",i]
   Malig_scores=marker_data[target=="M",i]
