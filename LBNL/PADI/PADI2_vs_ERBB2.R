working_dir="C:/Users/Obi/Documents/My Dropbox/Projects/PADI/v3/"
setwd(working_dir)

#Gene data file(s)
#ERBB2_file="ENSG00000141736.txt"
#PADI2_file="ENSG00000117115.txt"

#Cell line info
celllinefile="C:/Users/Obi/Documents/My Dropbox/Projects/PADI/cell_line_info.2.txt"
...
#Get PADI2/ERBB2 gene data, and sort according to subtype order determined above
...
PADI4_data_sorted=data[which(feat_data[,"Seq_Name"]=="PADI4"),lib_names_sorted]

#Also get expression status for these genes and summarize
ERBB2_exp_status_sorted=exp_status[which(feat_data[,"Seq_Name"]=="ERBB2"),lib_names_sorted]
PADI2_exp_status_sorted=exp_status[which(feat_data[,"Seq_Name"]=="PADI2"),lib_names_sorted]
PADI4_exp_status_sorted=exp_status[which(feat_data[,"Seq_Name"]=="PADI4"),lib_names_sorted]
...
paste(sum(PADI4_exp_status_sorted[subtypes_sorted=="Basal"]), "/", length(which(subtypes_sorted=="Basal")), "Basal Expressed")
paste(sum(PADI4_exp_status_sorted[subtypes_sorted=="Basal_NM"]), "/", length(which(subtypes_sorted=="Basal_NM")), "Basal_NM Expressed")
paste(sum(PADI4_exp_status_sorted[subtypes_sorted=="ClaudinLow"]), "/", length(which(subtypes_sorted=="ClaudinLow")), "ClaudinLow Expressed")
paste(sum(PADI4_exp_status_sorted[subtypes_sorted=="Luminal"]), "/", length(which(subtypes_sorted=="Luminal")), "Luminal Expressed")
...
PADI4_data_log2 = log2(PADI4_data_sorted+1)

#Calculate correlations between PADI2 and ERBB2
cors=matrix(NA, nrow=4, ncol=2, dimnames = list(unique(subtypes_sorted),c("Spearman_rho", "Spearman_p")))
spearman_all=cor.test(x=as.numeric(ERBB2_data_log2), y=as.numeric(PADI2_data_log2), method="spearman")
spearman_Basal=cor.test(x=as.numeric(ERBB2_data_log2[which(subtypes_sorted=="Basal")]), y=as.numeric(PADI2_data_log2[which(subtypes_sorted=="Basal")]), method="spearman")
spearman_Basal_NM=cor.test(x=as.numeric(ERBB2_data_log2[which(subtypes_sorted=="Basal_NM")]), y=as.numeric(PADI2_data_log2[which(subtypes_sorted=="Basal_NM")]), method="spearman")
spearman_Luminal=cor.test(x=as.numeric(ERBB2_data_log2[which(subtypes_sorted=="Luminal")]), y=as.numeric(PADI2_data_log2[which(subtypes_sorted=="Luminal")]), method="spearman")
spearman_Claudin=cor.test(x=as.numeric(ERBB2_data_log2[which(subtypes_sorted=="ClaudinLow")]), y=as.numeric(PADI2_data_log2[which(subtypes_sorted=="ClaudinLow")]), method="spearman")
spearman_notBasal=cor.test(x=as.numeric(ERBB2_data_log2[which(subtypes_sorted!="Basal")]), y=as.numeric(PADI2_data_log2[which(subtypes_sorted!="Basal")]), method="spearman")
...
pdf(file="PADI2_ERBB2_cor_by_subtype.pdf", width=10, height=7.5)
...
#Create plot to represent correlation between PADI2 and ERBB2
#break into panels for each subtype
i=1
subtype=unique(subtypes_sorted)[i]
ERBB2_data_log2_sub=ERBB2_data_log2[subtypes_sorted==subtype]
PADI2_data_log2_sub=PADI2_data_log2[subtypes_sorted==subtype]
lib_names_sub=names(ERBB2_data_log2_sub)
plot(x=(1:length(lib_names_sub)), y=as.numeric(ERBB2_data_log2_sub), type="n", xlim=c(1,length(lib_names_sub)), ylim=c(0,max(c(as.numeric(ERBB2_data_log2),as.numeric(PADI2_data_log2)))+1), xlab=NA, ylab="Log2 Gene Expression Level", xaxt="n", bty="n")
axis(1, at=1:length(lib_names_sub), labels=lib_names_sub, las=2, cex.axis=0.9)
points(x=(1:length(lib_names_sub)), y=as.numeric(ERBB2_data_log2_sub), col="blue", pch=18, cex=1.6)
points(x=(1:length(lib_names_sub)), y=as.numeric(PADI2_data_log2_sub), col="red", pch=15, cex=1.2)
lines(x=(1:length(lib_names_sub)), y=as.numeric(ERBB2_data_log2_sub), col="blue", lwd=2)
...
for (i in 2:length(unique(subtypes_sorted))){
 ...
 PADI2_data_log2_sub=PADI2_data_log2[subtypes_sorted==subtype]
 lib_names_sub=names(ERBB2_data_log2_sub)
 plot(x=(1:length(lib_names_sub)), y=as.numeric(ERBB2_data_log2_sub), type="n", xlim=c(1,length(lib_names_sub)), ylim=c(0,max(c(as.numeric(ERBB2_data_log2),as.numeric(PADI2_data_log2)))+1), xlab=NA, ylab="Log2 Gene Expression Level", xaxt="n", yaxt="n", bty="n")
 axis(1, at=1:length(lib_names_sub), labels=lib_names_sub, las=2, cex.axis=0.9)
 points(x=(1:length(lib_names_sub)), y=as.numeric(ERBB2_data_log2_sub), col="blue", pch=18, cex=1.6)
 points(x=(1:length(lib_names_sub)), y=as.numeric(PADI2_data_log2_sub), col="red", pch=15, cex=1.2)
 lines(x=(1:length(lib_names_sub)), y=as.numeric(ERBB2_data_log2_sub), col="blue", lwd=2)
...
#Get PADI2/ERBB2 gene data, and sort according to ERBB2 order determined above
ERBB2_data_sorted2=data[which(feat_data[,"Seq_Name"]=="ERBB2"),lib_names_sorted2]
PADI2_data_sorted2=data[which(feat_data[,"Seq_Name"]=="PADI2"),lib_names_sorted2]

#Convert values to log2, unless they are 0 in which case set them to 0
ERBB2_data_sorted2_log2 = log2(ERBB2_data_sorted2+1)
PADI2_data_sorted2_log2 = log2(PADI2_data_sorted2+1)

#Calculate correlations between PADI2 and ERBB2
spearman_ERBB2pos=cor.test(x=as.numeric(ERBB2_data_sorted2_log2[which(ERBB2_sorted=="Amp")]), y=as.numeric(PADI2_data_sorted2_log2[which(ERBB2_sorted=="Amp")]), method="spearman")
spearman_ERBB2neg=cor.test(x=as.numeric(ERBB2_data_sorted2_log2[which(ERBB2_sorted=="NoAmp")]), y=as.numeric(PADI2_data_sorted2_log2[which(ERBB2_sorted=="NoAmp")]), method="spearman")

pdf(file="PADI2_ERBB2_cor_by_ERBB2.pdf", width=10, height=7.5)
...
ERBB2_data_log2_ERBB2pos=ERBB2_data_sorted2_log2[ERBB2_sorted=="Amp"]
PADI2_data_log2_ERBB2pos=PADI2_data_sorted2_log2[ERBB2_sorted=="Amp"]
lib_names_ERBB2pos=names(ERBB2_data_log2_ERBB2pos)
plot(x=(1:length(lib_names_ERBB2pos)), y=as.numeric(ERBB2_data_log2_ERBB2pos), type="n", xlim=c(1,length(lib_names_ERBB2pos)), ylim=c(0,max(c(as.numeric(ERBB2_data_log2),as.numeric(PADI2_data_log2)))+1), xlab=NA, ylab="Log2 Gene Expression Level", xaxt="n", bty="n")
axis(1, at=1:length(lib_names_ERBB2pos), labels=lib_names_ERBB2pos, las=2, cex.axis=0.9)
points(x=(1:length(lib_names_ERBB2pos)), y=as.numeric(ERBB2_data_log2_ERBB2pos), col="blue", pch=18, cex=1.6)
points(x=(1:length(lib_names_ERBB2pos)), y=as.numeric(PADI2_data_log2_ERBB2pos), col="red", pch=15, cex=1.2)
lines(x=(1:length(lib_names_ERBB2pos)), y=as.numeric(ERBB2_data_log2_ERBB2pos), col="blue", lwd=2)
...
par(mar=c(7,2,3,1)) #bottom, left, top, right
ERBB2_data_log2_ERBB2neg=ERBB2_data_sorted2_log2[ERBB2_sorted=="NoAmp"]
PADI2_data_log2_ERBB2neg=PADI2_data_sorted2_log2[ERBB2_sorted=="NoAmp"]
lib_names_ERBB2neg=names(ERBB2_data_log2_ERBB2neg)
plot(x=(1:length(lib_names_ERBB2neg)), y=as.numeric(ERBB2_data_log2_ERBB2neg), type="n", xlim=c(1,length(lib_names_ERBB2neg)), ylim=c(0,max(c(as.numeric(ERBB2_data_log2),as.numeric(PADI2_data_log2)))+1), xlab=NA, ylab="Log2 Gene Expression Level", xaxt="n", yaxt="n", bty="n")
axis(1, at=1:length(lib_names_ERBB2neg), labels=lib_names_ERBB2neg, las=2, cex.axis=0.9)
points(x=(1:length(lib_names_ERBB2neg)), y=as.numeric(ERBB2_data_log2_ERBB2neg), col="blue", pch=18, cex=1.6)
points(x=(1:length(lib_names_ERBB2neg)), y=as.numeric(PADI2_data_log2_ERBB2neg), col="red", pch=15, cex=1.2)
lines(x=(1:length(lib_names_ERBB2neg)), y=as.numeric(ERBB2_data_log2_ERBB2neg), col="blue", lwd=2)
