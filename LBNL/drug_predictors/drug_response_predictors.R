# Copyright 2012 Obi Griffith

# drug_response_predictors.R is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
# 
# drug_response_predictors.R is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
# 
# You should have received a copy of the GNU General Public License
# along with drug_response_predictors.R. If not, see <http://www.gnu.org/licenses/>.
...
outdir="C:/Users/Obi/Documents/My Dropbox/drug_predictors/RFpredictors/allDrugs/core_alldata_imputed/"
RF_model_outdir="C:/Users/Obi/Documents/My Dropbox/drug_predictors/RFpredictors/allDrugs/core_alldata_imputed/RFmodels/"
setwd(outdir)
...
#datafile="C:/Users/Obi/Documents/My Dropbox/drug_predictors/combined/BCCL_combined_data.3.txt"
datafile="C:/Users/Obi/Documents/My Dropbox/drug_predictors/combined/BCCL_combined_data.3.COSMICextended.imp.txt"
...
GI50meanfile="C:/Users/Obi/Documents/My Dropbox/drug_predictors/drugdata/GI50meanThresholds.csv"
...
predictor_perf = data.frame(cbind(drugs, N=NA, sens=NA, spec=NA, acc=NA, err=NA, res_err=NA, sen_err=NA, AUC=NA, opt_feat_num=NA), stringsAsFactors=FALSE)
...
result <- replicate(5, rfcv(t(data_nonNA), target, cv.fold=5, scale="log", step=0.75, mtry=function(p) max(1, floor(sqrt(p))), recursive=FALSE), simplify=FALSE)
error.cv <- sapply(result, "[[", "error.cv")
mean.error.cv=signif(rowMeans(error.cv), digits=2)
min_error=min(mean.error.cv)
opt_feat_num=min(as.numeric(names(which(mean.error.cv==min_error))))
...
save(rf_model_opt, file=paste(RF_model_outdir,rf_model_file_opt,sep=""))
...
matplot(result[[1]]$n.var, cbind(rowMeans(error.cv), error.cv), type="l",lwd=c(2, rep(1, ncol(error.cv))), col=c("red",rep("black", dim(error.cv)[2])), lty=1, log="x",xlab="Number of features", ylab="CV Error")
...
#legend("bottomleft", legend=c("RNAseq","ExonArray","U133A","ExomeSeq","ManMut","CNV","RPPA","WB","Meth"), title="Data type", fill=colors, border="white", bty="n", cex=0.7)
legend("bottomleft", legend=c("RNAseq","ExonArray","U133A","COSMIC","CNV","RPPA","Meth"), title="Data type", fill=colors, border="white", bty="n", cex=0.7)
legend("top", legend=c("Luminal","Basal","Claudin-low","ERBB2Amp"), fill=c("black","red","green","blue"), title="Subtype", border="white",  bty="n",cex=0.7)
#dev.off()
