...
drug_annotations_file="/Users/obigriffith/Dropbox/drug_predictors/drugdata/CompoundsTargets.csv"
...
clin_data_file="/Users/obigriffith/Dropbox/drug_predictors/TCGA/ClinicalData 2 8 12-1.clean.txt"
...
subtype_file="/Users/obigriffith/Dropbox/drug_predictors/TCGA/UNC.TCGA.BRCA.ExpClusters.20110922_updatedSampleIDsV2.clean.txt"
...
#Load mutation calls for TP53/PI3KCA
#mutation_file="C:/Users/Obi/Documents/My Dropbox/drug_predictors/TCGA/Mutations/genome.wustl.edu_BRCA.IlluminaGA_DNASeq.Level_2.3.2.0.somatic.maf.clean.txt"
mutation_file="/Users/obigriffith/Dropbox/drug_predictors/TCGA/Mutations/genome.wustl.edu_BRCA.IlluminaGA_DNASeq.Level_2.3.2.0.somatic.maf.clean.txt"
mutationdata=read.table(mutation_file,sep="\t",header=TRUE,as.is=c(1,9,16),skip=1,row.names=NULL)
mutation_calls=mutationdata[,c(c("Hugo_Symbol","Variant_Classification","Tumor_Sample_Barcode"))]
mutation_calls[,"Tumor_Sample_Barcode"]=substr(mutation_calls[,"Tumor_Sample_Barcode"],1,12)
all_mut_sample=unique(mutation_calls[,"Tumor_Sample_Barcode"])
TP53mut_samples=unique(mutation_calls[which(mutation_calls[,"Hugo_Symbol"]=="TP53" & mutation_calls[,"Variant_Classification"]!="Silent"),"Tumor_Sample_Barcode"])
PIK3CAmut_samples=unique(mutation_calls[which(mutation_calls[,"Hugo_Symbol"]=="PIK3CA" & mutation_calls[,"Variant_Classification"]!="Silent"),"Tumor_Sample_Barcode"])

#Create dataframe of mutation statues (yes/no)
mutation_status=data.frame(TP53=rep(0,length(all_mut_sample)), PIK3CA=rep(0,length(all_mut_sample)), stringsAsFactors=FALSE, row.names=all_mut_sample)
mutation_status[TP53mut_samples,"TP53"]=1
mutation_status[PIK3CAmut_samples,"PIK3CA"]=1

#Load results file
#path="C:/Users/Obi/Documents/My Dropbox/drug_predictors/TCGA/RtoolboxResults/Results/"
path="/Users/obigriffith/Dropbox/drug_predictors/TCGA/RtoolboxResults/Results/"
...
bestheatmapfile_rescaled="TCGA_results_ExpMethCNV_heatmap_best_rescaled.pdf"
bestheatmapfile_rescaled2="TCGA_results_ExpMethCNV_heatmap_best_rescaled.2.pdf"
...
Nutlin3AVsP53file="TCGA_results_ExpMethCNV_Nutlin3AVsP53.pdf"
...
drug_group=drug_annotations[drugs,4]
drug_names=drug_annotations[drugs,2]
P53status=mutation_status[patients,"TP53"]
PIK3CAstatus=mutation_status[patients,"PIK3CA"]

#Set colors for color sidebars
drug_colors=drug_group
drug_colors[drug_colors==1]="darkorchid"
drug_colors[drug_colors==2]="darkred"
...
P53colors=P53status
P53colors[P53colors=="1"]="black"
P53colors[P53colors=="0"]="white"
P53colors[is.na(P53colors)]="grey"

PIK3CAcolors=PIK3CAstatus
PIK3CAcolors[PIK3CAcolors=="1"]="black"
PIK3CAcolors[PIK3CAcolors=="0"]="white"
PIK3CAcolors[is.na(PIK3CAcolors)]="grey"
...
clab=cbind(subtype_colors,PIK3CAcolors,P53colors,Mcolors,Ncolors,Tcolors,HER2_colors,PR_colors,ER_colors)
colnames(rlab)=c("Class","")
colnames(clab)=c("Subtype","PIK3CA","TP53","M","N","T","HER2","PR","ER")

pdf(file=bestheatmapfile, height=7.5, width=10)
main_title="TCGA Breast Tumor Drug Response Predictions"
par(cex.main=1)
heatmap.3(t(prob_matrix2), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none", dendrogram="both", margins=c(4,14), Rowv=TRUE, Colv=TRUE, ColSideColors=clab, RowSideColors=rlab, symbreaks=FALSE, key=TRUE, symkey=FALSE, density.info="none", trace="none", main=main_title, labCol=FALSE, labRow=drug_names, cexRow=1, col=rev(heat.colors(75)), NumColSideColors=9, KeyValueName="Prob. Response",)
...
legend("left",legend=c("Targeted","Chemo"), fill=c("darkorchid","darkred"), border=FALSE, bty="n", y.intersp = 0.7, cex=0.7)
dev.off()


### Heatmap 3 - All drugs that pass AUC cutoff and some minimal probability of response
### Probabilities re-scaled to reflect drug-by-drug cutoffs

#Load custom drug response classes
#prob_classes_file="C:/Users/Obi/Documents/My Dropbox/drug_predictors/TCGA/RtoolboxResults/Results/TCGA_results_ExpMethCNV_prob_classes_filtered.txt"
prob_classes_file="/Users/obigriffith/Dropbox/drug_predictors/TCGA/RtoolboxResults/Results/TCGA_results_ExpMethCNV_prob_classes_filtered.txt"
prob_classes=read.table(prob_classes_file, sep="\t", header=TRUE, row.names=1)

#Check to make sure prob and prob_classes matrix have matching rows and columns
cbind(colnames(prob_matrix2),colnames(prob_classes))
cbind(rownames(prob_matrix2),rownames(prob_classes))

#Set colnames for prob_classes to same (clean version) as for prob matrix
colnames(prob_classes)=colnames(prob_matrix2)

#Rescale data
#Take all probability values (across all drugs) for each class and rescale
prob_matrix_rescaled=prob_matrix2

for (i in 1:length(colnames(prob_matrix2))){
 res=which(prob_classes[,i]=="res")
 int=which(prob_classes[,i]=="int")
 sens=which(prob_classes[,i]=="sens")
 prob_matrix_rescaled[res,i]=rescale(prob_matrix2[res,i], to=c(0,0.333333), from=range(x=prob_matrix2[prob_classes=="res"]), clip = TRUE) 
 prob_matrix_rescaled[int,i]=rescale(prob_matrix2[int,i], to=c(0.333333,0.666666), from=range(x=prob_matrix2[prob_classes=="int"]), clip = TRUE) 
 prob_matrix_rescaled[sens,i]=rescale(prob_matrix2[sens,i], to=c(0.666666,1), from=range(x=prob_matrix2[prob_classes=="sens"]), clip = TRUE) 
}

pdf(file=bestheatmapfile_rescaled, height=7.5, width=10)
main_title="TCGA Breast Tumor Drug Response Predictions"
par(cex.main=1)
colors=rev(heat.colors(103))[c(1:31,37:67,73:103)]
heatmap.3(t(prob_matrix_rescaled), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none", dendrogram="both", margins=c(4,14), Rowv=TRUE, Colv=TRUE, ColSideColors=clab, RowSideColors=rlab, symbreaks=FALSE, key=TRUE, symkey=FALSE, density.info="none", trace="none", main=main_title, labCol=FALSE, labRow=drug_names, cexRow=1, col=colors, breaks=length(colors)+1, NumColSideColors=9, KeyValueName="Prob. Response",)
legend("topright",legend=c("Basal","LumA","LumB","Her2","Claudin","Normal","","Positive","Negative","NA","","","Targeted","Chemo"), fill=c("red","blue","cyan","pink","yellow","green","white","black","white","grey","white","white","darkorchid","darkred"), border=FALSE, bty="n", y.intersp = 0.7, cex=0.7)
dev.off()

pdf(file=bestheatmapfile_rescaled2, height=7.5, width=10)
main_title="TCGA Breast Tumor Drug Response Predictions"
par(cex.main=1)
#colors=c(color.scale(1:30,extremes=c("#FFFFFF","#FFFFB2")),color.scale(1:30,extremes=c("#FED976","#FD8D3C")),color.scale(1:30,extremes=c("#FC4E2A","#B10026")))
colors=c(color.scale(1:30,extremes=c("#FFFFFF","#FED976")),color.scale(1:30,extremes=c("#FEB24C","#FC4E2A")),color.scale(1:30,extremes=c("#E31A1C","#800026")))
heatmap.3(t(prob_matrix_rescaled), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none", dendrogram="both", margins=c(4,14), Rowv=TRUE, Colv=TRUE, ColSideColors=clab, RowSideColors=rlab, symbreaks=FALSE, key=TRUE, symkey=FALSE, density.info="none", trace="none", main=main_title, labCol=FALSE, labRow=drug_names, cexRow=1, col=colors, breaks=length(colors)+1, NumColSideColors=9, KeyValueName="Prob. Response")
legend("topright",legend=c("Basal","LumA","LumB","Her2","Claudin","Normal","","Positive","Negative","NA","","","Targeted","Chemo"), fill=c("red","blue","cyan","pink","yellow","green","white","black","white","grey","white","white","darkorchid","darkred"), border=FALSE, bty="n", y.intersp = 0.7, cex=0.7)
dev.off()
...
patients=rownames(prob_matrix2)
drugs=colnames(prob_matrix2)
tests=c("ER","PR","HER2","P53","PIK3CA","T","N","M","Subtype","Stage")
...
for (i in 1:length(drugs)){
  ...
  P53status.wilcox=wilcox.test(x=drug_data[which(P53status=="1")], y=drug_data[which(P53status=="0")], alternative="two.sided")
  PIK3CAstatus.wilcox=wilcox.test(x=drug_data[which(PIK3CAstatus=="1")], y=drug_data[which(PIK3CAstatus=="0")], alternative="two.sided")
  ...
  stat_results[drug,"P53"]=P53status.wilcox$p.value
  stat_results[drug,"PIK3CA"]=PIK3CAstatus.wilcox$p.value
  ...
  stat_results_long[which(stat_results_long[,"Drug"]==drug & stat_results_long[,"ClinVar"]=="P53"),"P-value"]=P53status.wilcox$p.value
  stat_results_long[which(stat_results_long[,"Drug"]==drug & stat_results_long[,"ClinVar"]=="PIK3CA"),"P-value"]=PIK3CAstatus.wilcox$p.value
...
combined_data=cbind(patients,prob_matrix2[patients,],rawclindata[patients,],P53status,PIK3CAstatus)
...
#Nutlin3A vs TP53 status
data=combined_data[which(combined_data[,"P53status"]=="1" | combined_data[,"P53status"]=="0"),c("Nutlin 3a","P53status")]
colnames(data)[1]="Nutlin3a"
data[which(data[,"P53status"]==1),"P53status"]="Mutant"
data[which(data[,"P53status"]==0),"P53status"]="WT"

pdf(file=Nutlin3AVsP53file, height=7.5, width=10)
ggplot(data, aes(Nutlin3a, fill = P53status)) + geom_density(alpha = 0.2) + xlab("Nutlin 3A Response Prob") + ylab("Density") + opts(title="Predicted Nutlin 3A response by P53 status")
dev.off()
