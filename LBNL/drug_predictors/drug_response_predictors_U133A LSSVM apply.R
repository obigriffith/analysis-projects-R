...
predictor_perf = data.frame(cbind(drugs_interest, N=NA, meanAUCtest=NA, stdAUCtest=NA, meanAUCtrain=NA, stdAUCtrain=NA, meanGamma=NA, stdGamma=std, meanGI50=NA), stringsAsFactors=FALSE)
...
for (drug in drugs_interest) {
	...
	RandomizationLoop <- function(rand) {
		...
		GammaLoop <- function(gamma) {
			InternalCV <- function(fold) {
				Vector=array(data=NA, dim=length(traintarget_rand))
				...
				Vector[Test]=prediction_latent
				return(Vector)
			}
			LatentMatrix=sapply(c(1:5),InternalCV)
			indices_NA=which(is.na(LatentMatrix))
			LatentMatrix[indices_NA]=0
			LatentVector=rowSums(LatentMatrix)
			
			# Calculation of 5-fold CV test performance for each gamma
			pred=prediction(LatentVector,traintarget_rand)
			perf_AUC=performance(pred,"auc")
			AUC=perf_AUC@y.values[[1]]
		}
		
		AUCvector=sapply(GammaVector,GammaLoop)
		...
		AUCvectorRefine=sapply(GammaVectorRefine,GammaLoop)
		...
		return(list(AUCrand,AUCmax,GammaOpt))
	} #end of randomization loop
	
	list_dummy=sapply(c(1:nbRandomizations),RandomizationLoop)
	AUCrand_test=list_dummy[1,]
	AUCrand_train=list_dummy[2,]
	GammaRand=list_dummy[3,]
	...
	predictor_perf[drug,"meanGamma"]=mean(GammaRand)
	predictor_perf[drug,"stdGamma"]=std(t(GammaRand))
