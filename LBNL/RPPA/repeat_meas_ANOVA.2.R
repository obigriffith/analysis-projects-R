...
ANOVA_outfile="synergy_vs_nonsynergy_all_ANOVA.2.txt"
...
ANOVA_outfile="synergy_vs_nonsynergy_HER2amp_ANOVA.2.txt"
...
wf1 = as.character(data[,"treatment"])
wf2 = as.character(data[,"time_hrs"])
bf1 = as.character(data[,"Synergy_status"])
sid = as.character(data[,"line_id"])
...
for (i in 1:length(proteins)){
 ...
 names(xdf)=c("abundance", "treatment", "time_hrs", "synergy", "line_id")

 #Calculate ANOVA for mixed within-between factors
 ezout = ezANOVA(data=xdf, dv=.(abundance), sid=.(line_id), within=.(treatment, time_hrs), between=.(synergy)) 
 ...
 ANOVA_results["treatment*synergy",proteins[i]]=ezout$ANOVA[ezout$ANOVA[,"Effect"]=="synergy:treatment","p"]
}
