...
clindatafile_random_train="filtered_combined_data_anno.final.train.2.txt"
clindatafile_random_test="filtered_combined_data_anno.final.test.2.txt"
...
for (i in 1:length(studies)){
  ...
  #break no10yrFU/10yrFUrelapse/10yrFUnorelapse into 2/3 training 1/3 test
  #First, count number of relapses and no relapses for study
  num_no10yrFU=length(rownames(data_sub_no10yrFU))
  ...
  num_test_no10yrFU=round(num_no10yrFU*1/3)
  ...
  random_no10yrFU=sample(x=num_no10yrFU, size=num_no10yrFU, replace = FALSE, prob = NULL)
  ...
  test=c(test, as.numeric(rownames(data_sub_no10yrFU[random_no10yrFU[1:num_test_no10yrFU],])))
  training=c(training, as.numeric(rownames(data_sub_no10yrFU[random_no10yrFU[(num_test_no10yrFU+1):num_no10yrFU],])))
