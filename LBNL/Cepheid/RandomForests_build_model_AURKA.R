...
setwd("/Users/ogriffit/Dropbox/LBNL/Projects/Cepheid/analyzing/analysis_final2/RandomForests/train_survival/unbalanced/final_100k_trees/AURKA/")
...
outfile_top8="Cepheid_RFoutput_top8.txt"
...
#Get pre-selected optimized top 8 predictor variables
top8opt_probes=c("208079_s_at")
top8opt_data=rawdata[rawdata[,1]%in%top8opt_probes,]

predictor_data=t(top8opt_data[,4:length(header)]) #Top8 optimized list
predictor_names=top8opt_data[,3] #gene symbol
...
predictor_data_10yr=predictor_data[cases_10yr,,drop=FALSE]

#Also put aside cases without 10yr FU
cases_no10yr = is.na(clindata[,"X10yr_relapse"])
clindata_no10yr=clindata[cases_no10yr,]
predictor_data_no10yr=predictor_data[cases_no10yr,,drop=FALSE]
...
save(rf_model, file="RF_model_8gene_optimized")
